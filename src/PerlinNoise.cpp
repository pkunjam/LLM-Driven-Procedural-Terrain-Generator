...
// Initialize the static permutation array.
...
{
    // Resize the permutation vector to 512 and duplicate the permutation array.
    p.resize(512);
    for (int i = 0; i < 256; ++i)
        p[256 + i] = p[i] = permutation[i];
}

float PerlinNoise::noise(float x, float y, int octaves, float persistence)
{
    float total = 0.0f;
...
float PerlinNoise::singleNoise(float x, float y)
{
